...
<div class="container mt-4">
    ...
    <form action="{{ route('staff.tbdots.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" name="date_of_diagnosis" value="{{ old('date_of_diagnosis') }}" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="tb_type" class="form-control" required>
                ...
                <option value="pulmonary" {{ old('tb_type') == 'pulmonary' ? 'selected' : '' }}>Pulmonary</option>
                <option value="extra_pulmonary" {{ old('tb_type') == 'extra_pulmonary' ? 'selected' : '' }}>Extra Pulmonary</option>
            </select>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="treatment_category" class="form-control" required>
                ...
                <option value="category_1" {{ old('treatment_category') == 'category_1' ? 'selected' : '' }}>Category 1</option>
                <option value="category_2" {{ old('treatment_category') == 'category_2' ? 'selected' : '' }}>Category 2</option>
                <option value="category_3" {{ old('treatment_category') == 'category_3' ? 'selected' : '' }}>Category 3</option>
            </select>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input type="date" class="form-control" name="treatment_start_date" value="{{ old('treatment_start_date') }}" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="treatment_end_date" class="form-label">Treatment End Date</label>
                <input type="date" class="form-control" name="treatment_end_date" value="{{ old('treatment_end_date') }}">
            </div>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="treatment_status" class="form-control" required>
                <option value="ongoing" {{ old('treatment_status') == 'ongoing' ? 'selected' : '' }}>Ongoing</option>
                <option value="completed" {{ old('treatment_status') == 'completed' ? 'selected' : '' }}>Completed</option>
                <option value="defaulted" {{ old('treatment_status') == 'defaulted' ? 'selected' : '' }}>Defaulted</option>
                <option value="failed" {{ old('treatment_status') == 'failed' ? 'selected' : '' }}>Failed</option>
                <option value="died" {{ old('treatment_status') == 'died' ? 'selected' : '' }}>Died</option>
                <option value="transferred_out" {{ old('treatment_status') == 'transferred_out' ? 'selected' : '' }}>Transferred Out</option>
            </select>
        </div>
