            ...
            <div class="row">
                <div class="col mr-0">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <a href="{{ route('admin.animal-bite.create') }}" class="btn btn-primary btn-sm">
                            Add Case
                        </a>
                    </div>
                        </div>
                            ...
                            <tbody id="animalBiteTableBody">
                                @forelse($animalBiteCases as $case)
                                    <tr style="cursor: pointer;"
                                        onclick="window.location='{{ route('admin.animal-bite.show', $case->id) }}'">
                                        {{-- patient name from appointments if available --}}
                                        <td>{{ $case->appointment->name ?? 'N/A' }}</td>
                                        <td>{{ $case->date_of_incident ? \Carbon\Carbon::parse($case->date_of_incident)->format('Y/m/d') : '-' }}
                                        </td>
...
<script>
    // Client-side search filtering for Animal Bite Cases
