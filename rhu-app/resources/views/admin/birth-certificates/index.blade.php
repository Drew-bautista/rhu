            ...
            <div class="row">
                <div class="col mr-0">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <a href="{{ route('admin.birth-certificates.create') }}" class="btn btn-primary btn-sm">
                ...
                <div class="card-body">
                    <div class="table table-responsive">
                        <table class="table table-hover" id="birthCertificateTable" width="100%" cellspacing="0">
                            <thead>
                            ...
                            <tbody id="birthCertificateTableBody">
                                @forelse($birthCertificates as $certificate)
                                    <tr>
                                        <td>
                                            @if($certificate->registry_number)
                                                <span class="badge bg-info">{{ $certificate->registry_number }}</span>
                                            @else
                                                <span class="text-muted">Not Assigned</span>
                                            @endif
                                        </td>
                                        <td>
                                            <strong>{{ $certificate->child_full_name }}</strong><br>
                                            <small class="text-muted">{{ $certificate->child_sex }}</small>
                                        </td>
                                        <td>{{ $certificate->date_of_birth->format('M d, Y') }}</td>
                                        <td>{{ $certificate->place_of_birth }}</td>
                                        <td>{{ $certificate->mother_full_name }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.birth-certificates.show', $certificate->id) }}" 
                                                class="btn btn-info btn-sm" title="View">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.birth-certificates.edit', $certificate->id) }}" 
...
<script>
    ...
    $(document).ready(function () {
        ...
        const $tbody = $('#birthCertificateTableBody');
